...
# =========================================
# =========================================
# 小説詳細（tags / likes / episodes / author）
# =========================================
...
def get_novel_detail(
    ...
    is_premium = FORCE_ALL_PREMIUM or (
        bool(getattr(current_user, "is_premium", False)) if current_user else False
    )
...
@app.put("/api/users/me", response_model=schemas.ProfileRead)
def update_profile(
    ...
    if profile.email:
        existing = (
            db.query(models.User)
            .filter(models.User.email == profile.email)
            .first()
        )
        if existing:
...
def delete_novel(
    ...
    novel = db.query(models.Novel).filter(models.Novel.id == novel_id).first()
    if not novel:
        raise HTTPException(status_code=404, detail="小説が存在しません")
    ...
    # 安全のため Episode 削除
    db.execute(
        text("DELETE FROM episodes WHERE novel_id = :nid"),
        {"nid": novel_id},
    )
    # Novel 自体削除
    db.execute(
        text("DELETE FROM novels WHERE id = :nid"),
        {"nid": novel_id},
    )
    db.commit()
...
def update_profile(
    ...
    # 生年月日 "YYYY-MM-DD" 形式
    if "birth_date" in payload:
        birth_str = payload["birth_date"]
        if birth_str:
            try:
                user.birth_date = date.fromisoformat(birth_str)
            except ValueError:
