...
async function loadLibrary(){
 ...
 novels.forEach(n=>{const row=document.createElement("div");row.className="library-row";const star=document.createElement("button");star.className="bookmark-star";star.textContent=n.bookmarked?"★":"☆";star.title=n.bookmarked?"ブックマーク解除":"ブックマーク";star.onclick=async e=>{e.stopPropagation();await window.lexis.saveLibraryNovel({...n,bookmarked:!n.bookmarked});await loadLibrary();};const b=document.createElement("button");b.className="library-item";b.textContent=(n.title||"タイトル未設定")+"　"+new Date(n.savedAt).toLocaleString();b.onclick=()=>openLibraryNovel(n);row.append(star,b);$("libraryList").appendChild(row);});
}
...
function updateDetailBookmark(){const b=$("libraryBookmark");if(b&&selectedLibraryNovel){b.textContent=selectedLibraryNovel.bookmarked?"★ ブックマーク済み":"☆ ブックマーク";b.title=selectedLibraryNovel.bookmarked?"ブックマーク解除":"ブックマーク";}}
function openLibraryNovel(n){selectedLibraryNovel=n;$("libraryList").style.display="none";$("libraryDetail").style.display="block";$("libraryTitleInput").value=n.title||"Lexis生成小説";$("libraryBody").value=n.body||"";$("libraryStatus").textContent="";setLibraryEditing(false);updateDetailBookmark();}
...
$("libraryBookmark")?.addEventListener("click",async()=>{if(!selectedLibraryNovel)return;const saved=await window.lexis.saveLibraryNovel({...selectedLibraryNovel,bookmarked:!selectedLibraryNovel.bookmarked});selectedLibraryNovel={...selectedLibraryNovel,...saved};updateDetailBookmark();$("libraryStatus").textContent=selectedLibraryNovel.bookmarked?"ブックマークしました。":"ブックマークを解除しました。";});
