...
export default function Home({
  ...
  const [followingFeedNovels, setFollowingFeedNovels] = useState([]);
  const [followingFeedLoading, setFollowingFeedLoading] = useState(false);
  const [followingFeedError, setFollowingFeedError] = useState("");
  const [followingTagsFeedNovels, setFollowingTagsFeedNovels] = useState([]);
  ...
  const applyRankingStateToUrl = (next = {}) => {
    const params = new URLSearchParams(location.search);
  ...
  const buildRankingPresetSearch = ({ sort: nextSort, period: nextPeriod, creativeType: nextCreativeType }) => {
    const params = new URLSearchParams(location.search);
  ...
  const creativeTypeLabel = (value) => {
    if (value === "fanfic") return "fanfic";
    return "original";
  };
  const renderNovelBadges = (novel) => (
    <>
  ...
  const renderNovelAuthorMeta = (novel) => (
  ...
  const formatRecommendationReasonLabel = (key) => {
  ...
  if (currentEffectiveSort !== "new") {
    activeFilterChips.push({
      ...
      text: t(
        ...
        { value: sortLabelMap[currentEffectiveSort] || currentEffectiveSort }
      ),
    });
  }
  ...
  const applyRankingTagFilter = (tagName) => {
  ...
  const applyRankingPreset = ({ sort: nextSort, period: nextPeriod, creativeType: nextCreativeType }) => {
                    ...
                    >
                      ...
                      {Array.isArray(novel.tags) && novel.tags.length > 0 && (
                        <div className="tag-chip-row">
                          {novel.tags.slice(0, 4).map((tagItem) => {
                            const tagName = String(tagItem?.name || "").trim();
