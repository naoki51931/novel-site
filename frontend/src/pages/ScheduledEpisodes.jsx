...
export default function ScheduledEpisodes() {
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      setError(e.message || t({ ja: "予約投稿一覧の取得に失敗しました", en: "Failed to load scheduled episodes." }));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUnschedule = async (episodeId) => {
    const token = localStorage.getItem("token");
    if (!token) {
      navigate("/login");
      return;
    ...
    setItems((prev) => prev.filter((item) => item.episode_id !== episodeId));
  };
