...
export default function Board() {
  ...
  const [replyPostingThreadId, setReplyPostingThreadId] = useState(null);
  const [deletingId, setDeletingId] = useState(null);
  const [isAdmin, setIsAdmin] = useState(false);
  const [error, setError] = useState("");
  const [replyTitleByThread, setReplyTitleByThread] = useState({});
  const [replyBodyByThread, setReplyBodyByThread] = useState({});
  ...
  const repliesByMainId = useMemo(() => {
    ...
    for (const post of posts) {
      ...
      grouped.get(parentId).push(post);
    }
  ...
  const handleReplySubmit = async (mainPost) => {
    ...
    try {
      ...
      setError(e.message || t({ ja: "返信に失敗しました", en: "Failed to reply." }));
    } finally {
  ...
  const handleSelectMainThread = (post) => {
