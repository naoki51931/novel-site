...
type AuthorDashboardData = {
  summary?: AuthorDashboardSummary | null;
  novels?: AuthorDashboardNovel[] | null;
};

type AuthorSeriesData = {
  series?: SeriesRow[] | null;
};

function TinyBarChart({ rows }: { rows: SeriesRow[] }) {
...
export default function AuthorDashboard() {
  ...
  const [selectedNovel, setSelectedNovel] = useState<AuthorDashboardNovel | null>(null);
  const [selectedDays, setSelectedDays] = useState(30);
  const [seriesLoading, setSeriesLoading] = useState(false);
  const [seriesError, setSeriesError] = useState("");
  const [seriesData, setSeriesData] = useState<AuthorSeriesData | null>(null);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        setError(getErrorMessage(error, t({ ja: "読み込み中にエラーが発生しました", en: "Failed to load." })));
      } finally {
        setLoading(false);
      }
    };
  ...
  const loadSeries = async (novel: AuthorDashboardNovel, days: number) => {
    const token = localStorage.getItem("token");
    ...
    try {
      ...
      setSeriesError(
        getErrorMessage(error, t({ ja: "PV推移の取得に失敗しました", en: "Failed to load daily metrics." }))
      );
      setSeriesData(null);
    } finally {
