...
export default function NewEpisode() {
  ...
  // 押絵は複数選択できるようにする
  const [illustItems, setIllustItems] = useState<IllustItem[]>([]); // [{file, caption, illustTag, metaTags}]
  ...
  const [assistCandidates, setAssistCandidates] = useState<string[]>([]);
  ...
  useEffect(() => {
    try {
      ...
      if (!raw) return;
      const draft = JSON.parse(raw) as EpisodeDraft;
  ...
  useEffect(() => {
    ...
    const prefill = (location?.state as { aiPrefill?: EpisodePrefill } | null | undefined)?.aiPrefill;
  ...
  const handleIllustFilesChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    // 追加形式（既存に追記）
    setIllustItems((prev: IllustItem[]) => {
      const usedTags = new Set(
        prev
          .map((it) => (it.illustTag || "").trim())
          .filter((tag: string) => tag.length > 0)
      );
      const next = files.map((f: File) => {
  ...
  const removeIllustItem = (index: number) => {
    setIllustItems((prev: IllustItem[]) => prev.filter((_, i) => i !== index));
  };

  const uploadCover = async (episodeId: number | string, token: string) => {
    if (!coverFile) return;
  ...
  const uploadIllusts = async (episodeId: number | string, token: string) => {
  ...
  const moveAssistSelection = (delta: number) => {
  ...
  const normalizeAssistCandidate = (text: string | null | undefined) => {
  ...
  const trimOverlapWithBodyTail = (bodyText: string | null | undefined, candidateText: string | null | undefined) => {
  ...
  const applyAssistCandidate = (candidate: string | null | undefined) => {
    if (!candidate) return;
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const candidates = Array.isArray(data?.candidates)
          ? data.candidates.map((x: unknown) => String(x || "").trim()).filter(Boolean)
          : [];
        if (!candidates.length) {
          setAssistCandidates([]);
          setAssistSelectedIndex(0);
        ...
        setAssistError(
          getErrorMessage(
            err,
            t({ ja: "候補の生成中にエラーが発生しました。", en: "An error occurred while generating candidates." })
          )
        );
      } finally {
        setAssistLoading(false);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      let data: CreatedEpisodeResponse | null = null;
      try {
        data = await res.json();
      } catch (_) {}
      ...
      setError(
        getErrorMessage(err, t({ ja: "投稿中にエラーが発生しました。", en: "An error occurred while posting." }))
      );
    } finally {
      setLoading(false);
    }
