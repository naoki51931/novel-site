...
const GIRLFRIEND_PRESET_POOL: CharacterPreset[] = [
  {
    name: "一ノ瀬 ひより",
...
const BOYFRIEND_PRESET_POOL: CharacterPreset[] = [
  {
    name: "神谷 蓮",
...
type ChatMode = "say" | "do";
type ImageMessageKind = "generated_images" | "uploaded_images";
...
type ChatMessage = {
  ...
  role: ChatRole;
  mode: ChatMode;
  is_auto_dialogue: boolean;
  content: string;
  speaker_name: string;
  model_name: string;
  message_owner_username?: string;
  is_generated_image: boolean;
  image_message_kind: ImageMessageKind;
...
type CharacterPreset = {
  ...
  personality: string;
  appearance: string;
  speech_gender: Extract<SpeechGender, "female" | "male">;
};

type EntryPreset = "" | "girlfriend" | "boyfriend";

type ParsedImageMessage = {
  images: GeneratedImage[];
  prompt: string;
  kind: ImageMessageKind;
  descriptions: string[];
};
...
function randomPick<T>(list: readonly T[] | null | undefined): T | null {
...
function getAiChatPresetFromLocation(locationLike: LocationLike | null | undefined): EntryPreset {
...
function normalizeSpeakerName(name: unknown) {
  return String(name || "").trim();
}

function parseGeneratedImageMessageContent(rawContent: unknown): ParsedImageMessage | null {
  ...
  try {
    ...
    const kind =
      String(parsed?.kind || "").trim() === "uploaded_images" ? "uploaded_images" : "generated_images";
...
function normalizeRelationshipMemoHistory(raw: unknown): RelationshipMemo[] {
  if (!Array.isArray(raw)) return [];
  const now = Date.now();
  const normalized: RelationshipMemo[] = [];
  const seen = new Set();
  raw.forEach((item: unknown, idx: number) => {
    const record = item && typeof item === "object" ? (item as Record<string, unknown>) : null;
    const text = compactText(record ? record.text : item);
    if (!text) return;
    const key = text.toLowerCase();
    if (seen.has(key)) return;
    seen.add(key);
    const useCountRaw = record ? Number(record.use_count) : NaN;
    const lastUsedRaw = record ? Number(record.last_used_at) : NaN;
    normalized.push({
      text,
...
function normalizeChatFetchErrorMessage(error: { message?: string; name?: string } | null | undefined, fallbackMessage: string, t: any) {
...
function isNetworkFetchError(error: { message?: string; name?: string } | null | undefined) {
...
function resolveImageUrl(rawUrl: unknown) {
...
function calcChatMessagesBytes(messages: unknown) {
  try {
...
async function fetchWithSingleRetry(url: RequestInfo | URL, options?: RequestInit, retryDelayMs = 350) {
  try {
...
function sanitizeImagePromptSourceText(text: unknown) {
  return compactText(text);
}

function extractConcreteImagePromptWords(text: unknown, limit = 8): string[] {
...
function uniqueTokens(tokens: unknown, limit = 12): string[] {
  const picked: string[] = [];
  const seen = new Set();
...
function splitPersonalityAndAppearance(rawText: unknown) {
...
function mergePersonalityWithAppearance(personalityText: unknown, appearanceText: unknown) {
...
function hashStringSeed(text: unknown) {
  const s = String(text || "");
  let h = 0;
...
function pickBySeed(items: readonly string[] | null | undefined, seed: number, offset = 0) {
...
function buildRandomAppearanceText(name: unknown, speechGender: SpeechGender = "auto") {
...
function deriveFanficAppearanceFromSources(
  characterName: unknown,
  sources: Array<{ title?: string; snippet?: string }> | unknown,
  fallback = ""
) {
...
function normalizeSpeechGender(value: unknown): SpeechGender {
...
function normalizeStoredToken(raw: unknown) {
...
function buildCharacterDraftStorageKey(characterId: unknown) {
  const id = String(characterId || "").trim();
  if (!id) return "";
  return `${AI_CHAT_CHARACTER_DRAFT_PREFIX}${id}`;
}

function loadCharacterChatDraft(characterId: unknown) {
  if (typeof window === "undefined") return null;
...
function hashGuestDraftText(text: unknown) {
...
function getByteLengthUtf8(text: unknown) {
  try {
...
function downloadGuestChatDraftJson(draft: any, rawText: any = "") {
...
function shouldBackupGuestDraft(rawText: unknown, options: { force?: boolean } = {}) {
...
function markGuestDraftBackedUp(hash: string, bytes = 0) {
...
function messageDiffSignature(m: Partial<ChatMessage> | null | undefined) {
...
function extractAppendOnlyDiffMessages(sourceMessages: unknown, existingMessages: unknown): ChatMessage[] {
  const src = (Array.isArray(sourceMessages) ? sourceMessages : []) as ChatMessage[];
  const existing = (Array.isArray(existingMessages) ? existingMessages : []) as ChatMessage[];
...
function normalizeStoredGuestMessage(raw: any): ChatMessage | null {
  ...
  const imageDescriptions = Array.isArray(raw.image_descriptions)
    ? raw.image_descriptions.map((d: any) => String(d || "").trim()).filter(Boolean)
    : [];
  return {
    id: null,
    role,
...
function buildGuestImportMessages(rawMessages: any): Array<Pick<ChatMessage, "role" | "mode" | "is_auto_dialogue" | "content">> {
  ...
  return list
    .map((m: any) => {
      ...
      if (
        ...
        content = `【ユーザー添付画像の説明】\n${m.image_descriptions.map((d: any, i: any) => `${i + 1}. ${String(d || "").trim()}`).filter(Boolean).join("\n")}`;
      }
    ...
    .filter(
      (
        item
      ): item is Pick<ChatMessage, "role" | "mode" | "is_auto_dialogue" | "content"> => Boolean(item)
    )
    .slice(-300);
}
...
export default function AiChatPage() {
  ...
  const [latestPromptPreview, setLatestPromptPreview] = useState<any>(null);
  const [previewLoading, setPreviewLoading] = useState(false);
  const [nextLineSuggestions, setNextLineSuggestions] = useState<string[]>([]);
  ...
  const [animeTitleCandidates, setAnimeTitleCandidates] = useState<string[]>([]);
  ...
  const [chatImageFiles, setChatImageFiles] = useState<any[]>([]);
  ...
  const [saveNameConflict, setSaveNameConflict] = useState<any>(null);
  const [relationshipMemoHistory, setRelationshipMemoHistory] = useState<RelationshipMemo[]>(() => {
    if (typeof window === "undefined") return [];
    try {
  ...
  const mapApiCharacter = (raw: any): SavedCharacter => {
  ...
  const formatCharacterNameWithIndex = (c: SavedCharacter) => {
  ...
  const normalizeAiNovelResponse = (data: unknown) => {
    if (!data || typeof data !== "object") return data;
    const record = data as Record<string, unknown>;
    const rawBody = String(record.body || "").trim();
    if (!rawBody) return data;
    const rawTitle = String(record.generated_title || "").trim();
  ...
  const buildScenePromptFromCurrentChat = () => {
    ...
    const pickByKeywords = (text: unknown, candidates: readonly string[], limit = 5) => {
  ...
  const normalizePromptTokensForTemplate = (value: unknown, fallback = "", limit = 8) => {
    const primary = extractConcreteImagePromptWords(value, limit);
    if (primary.length) return primary;
    return extractConcreteImagePromptWords(fallback, limit);
  };
  ...
  const buildCharacterOutputTemplate = () => {
    ...
    const traits = uniqueTokens(
      normalizePromptTokensForTemplate(
        personality,
  ...
  const withImageUrl = (url: unknown) => {
  ...
  const handleCreateNovelFromConversation = async () => {
    ...
    try {
      ...
      const generatedRaw = (await generateRes.json().catch(() => ({}))) as Record<string, unknown>;
      const generated = normalizeAiNovelResponse(generatedRaw || {}) as Record<string, unknown>;
      if (!generateRes.ok) {
        const generatedDetail =
          typeof generated?.detail === "string" ? generated.detail : "";
        throw new Error(
          generatedDetail ||
            t(
              {
  ...
  const uploadAdditionalChatImages = async () => {
    ...
    try {
      ...
      const descriptions = Array.isArray(data?.descriptions)
        ? data.descriptions.map((v: any) => String(v || "").trim()).filter(Boolean)
        : [];
      if (!images.length) {
        throw new Error(
      ...
      const imageDescriptionText =
        descriptions.length > 0
          ? `【ユーザー添付画像の説明】\n${descriptions.map((d: any, i: any) => `${i + 1}. ${d}`).join("\n")}`
          : t({
        ...
        .map((m) => ({
          ...
          content: (() => {
            const base = (
              ...
              ? `【ユーザー添付画像の説明】\n${m.image_descriptions.map((d: any, i: any) => `${i + 1}. ${d}`).join("\n")}`
              : m.content;
  ...
  const getSpeakerProfileByKey = (key: string) => {
    if (key === "you") {
      return {
        key: "you",
  ...
  const isStopCommand = (text: unknown) => {
  ...
  const createCastCharacter = (): CastCharacter => ({
  ...
  const updateCastCharacter = (key: string, patch: Partial<CastCharacter>) => {
    setCastCharacters((prev) =>
      prev.map((c) => (c.key === key ? { ...c, ...patch } : c))
    );
  };
  const getResolvedCastRelationship = (cast: CastCharacter) => {
    const typed = compactText(cast?.relationship || "");
    if (typed) return typed;
    return compactText(castRelationshipSelectMap?.[cast?.key] || "");
  };
  const rememberRelationshipMemo = (text: unknown) => {
  ...
  const applySavedCharacterToCast = (castKey: string, savedId: string) => {
  ...
  const removeCastCharacter = (key: string) => {
  ...
  const buildDuplicatedCastName = (baseName: unknown, names: string[]) => {
  ...
  const duplicateCastCharacter = (castKey: string) => {
  ...
  const toggleRandomSpeakerKey = (key: string, checked: boolean) => {
    setRandomSpeakerKeys((prev) => {
      if (checked) {
  ...
  const toggleAutoRandomSpeakerKey = (key: string, checked: boolean) => {
    setAutoRandomSpeakerKeys((prev) => {
      if (checked) {
        if (prev.includes(key)) return prev;
        return [...prev, key];
  ...
  const buildParticipantsContext = (speakerKey: any) => {
    const participants: any[] = [];
    const relationshipRules: any[] = [];
    const perspectiveRules: any[] = [];
    ...
    const castNamed = castCharacters
      .map((c: any, idx: any) => ({ ...c, __idx: idx + 1, __name: String(c.name || "").trim() }))
      .filter((c: any) => c.__name);
    const nameCounts: Record<string, number> = {};
    if (mainName) {
      nameCounts[mainName] = (nameCounts[mainName] || 0) + 1;
    }
    castNamed.forEach((c: any) => {
    ...
    const renderCastLabel = (name: any, idx: any) => {
      return nameCounts[name] > 1 ? `サブ${idx}(${name})` : `サブ${idx}:${name}`;
    };
    const formatProfileForContext = (text: any) => {
    ...
    castNamed.forEach((c: any) => {
      ...
      if (rel) {
        ...
        if (romanticKeywords.some((k: any) => rel.toLowerCase().includes(String(k).toLowerCase()))) {
  ...
  const buildRoleplayConstraint = (speakerName: any) => {
  ...
  const buildSpeechGenderConstraint = (speakerName: any, speechGender: any) => {
  ...
  const renderSpeechGenderButtons = (value: any, onChange: any) => (
  ...
  const openAnimeTitlePicker = async () => {
    ...
    try {
      ...
      const candidates = Array.isArray(data?.candidates)
        ? data.candidates.map((v: any) => String(v || "").trim()).filter(Boolean)
        : [];
      if (!candidates.length) {
  ...
  useEffect(() => {
      ...
      .map((m: any) => {
        ...
        return {
          ...
          image_descriptions: Array.isArray(m.image_descriptions)
            ? m.image_descriptions.map((d: any) => String(d || "").trim()).filter(Boolean)
            : [],
        };
      })
      .filter(Boolean);
    ...
    const hasMeaningfulState = !!(
      ...
      || trimmedPersonality
      || trimmedAppearance
      || normalizedMessages.some((m: any) => String(m?.content || "").trim())
    );
  ...
  useEffect(() => {
    ...
    const normalizedMessages = (Array.isArray(messages) ? messages : [])
      .slice(-500)
      .map((m: any) => normalizeStoredGuestMessage(m))
      .filter(Boolean);
    const payload = {
      version: 1,
      character_id: cid,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setSavedCharacters((prev: any) => [normalizedSaved, ...prev.filter((c: any) => c.id !== normalizedSaved.id)]);
  ...
  const uploadCharacterImage = async (characterId: string, file: File, token: string) => {
  ...
  const saveCharacter = (saveMode = "ask") => {
    const run = async () => {
      ...
      const saved: SavedCharacter = {
        ...
        recommendation_samples: Number(data.recommendation_samples || 0),
  ...
  const applySelectedCharacter = (id: string) => {
    if (!id) {
      setSelectedCharacterId("");
      setCharacterImageFile(null);
  ...
  const updatePersonalitySetting = () => {
    const run = async () => {
      ...
      setSavedCharacters((prev: any) => prev.map((c: any) => (c.id === updated.id ? updated : c)));
  ...
  const saveCastCharacter = (castKey: any) => {
    const run = async () => {
      const cast = castCharacters.find((c: any) => c.key === castKey);
      if (!cast) return;
      const token = getStoredAuthToken();
      if (!token) {
        throw new Error(t({ ja: "キャラ登録はログインが必要です。", en: "Login is required to save characters." }));
      ...
      const castSaved = savedCharacters.find((c: any) => c.id === String(cast.saved_id || ""));
  ...
  const continueAutoDialogue = async (explicitHistory: any) => {
    ...
    try {
      ...
      const assistantSpeechGender = normalizeSpeechGender(assistantProfile?.speech_gender);
      const token = getStoredAuthToken();
      const headers: Record<string, string> = { "Content-Type": "application/json" };
      if (token) {
        headers.Authorization = `Bearer ${token}`;
  ...
  const submitChat = async (overrideText: any = null) => {
    ...
    const resolvedSpeechGender = normalizeSpeechGender(assistantProfile?.speech_gender);
  ...
  const sendSelectedBubbleLine = (line: any) => {
    const text = compactText(line);
    if (!text || text === "「……」" || text === "候補を生成中…" || text === "Generating suggestions..." || loading) return;
    submitChat(text);
  };

  const applyServerChatMessages = (raw: any) => {
    const list = Array.isArray(raw) ? raw : [];
    setMessages(
      list.map((m: any): ChatMessage => {
        const role = m?.role === "assistant" ? "assistant" : "user";
        ...
        return {
          ...
          is_generated_image: !!parsedImage,
          image_message_kind: imageKind === "uploaded_images" ? "uploaded_images" : "generated_images",
  ...
  const removeGeneratedImageAt = (messageIndex: any, imageIndex: any) => {
    const run = async () => {
      ...
      setMessages((prev: any) => {
        ...
        if (!list.length) {
          return prev.filter((_: any, idx: any) => idx !== messageIndex);
        }
        const next = [...prev];
        next[messageIndex] = {
          ...current,
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const snapshot = messages.slice(-20).map((m: any) => ({
        role: m.role,
        content: m.content,
        mode: m.mode,
      }));
  ...
  useEffect(() => {
    ...
    const speakerSpeechGender = normalizeSpeechGender(userSpeakerProfile?.speech_gender);
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const list = Array.isArray(data?.suggestions)
          ? data.suggestions.map((s: any) => compactText(s)).filter(Boolean)
          : [];
        if (!list.length) {
          setNextLineSuggestions(fallback);
          return;
  ...
  useEffect(() => {
    if (!selectedCharacterId) return;
    if (showR18ByDisplaySetting) return;
    const selected = savedCharacters.find((c: any) => c.id === selectedCharacterId);
    if (selected?.is_r18) {
      setSelectedCharacterId("");
    }
  }, [selectedCharacterId, savedCharacters, showR18ByDisplaySetting]);

  const topNavRowStyle: any = isMobileViewport
    ? {
        display: "grid",
  ...
  const topNavButtonStyle: any = isMobileViewport
    ? {
        width: "100%",
        textAlign: "center",
        whiteSpace: "normal",
    ...
    <div className="ai-chat-page" style={{ maxWidth: 960, margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          {engagementSummary && Number(engagementSummary.sample_size || 0) > 0 ? (
            <>
              ...
              <div style={{ display: "grid", gap: 6, marginTop: 8 }}>
                {engagementMetricRows.map((row: any) => (
                  <div key={row.key}>
        ...
        >
          ...
          >
            ...
            <div style={{ display: "grid", gap: 8 }}>
              {animeTitleCandidates.map((title: any, idx: any) => (
      ...
      <div style={{ display: "grid", gap: 8, marginBottom: 12 }}>
        <label>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 4, flexWrap: "wrap" }}>
            <select
              ...
              onChange={(e: any) => applySelectedCharacter(e.target.value)}
            ...
            >
              ...
              {visibleSavedCharacters.map((c: any) => (
          ...
          <label style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8, fontSize: "0.92rem" }}>
            <input
              type="checkbox"
              checked={fanficMode}
              onChange={(e: any) => setFanficMode(e.target.checked)}
              disabled={loading || augmentLoading}
            />
            <span>
              {t({
        ...
        <div style={{ border: "1px solid #ddd", borderRadius: 8, padding: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            ...
            <button
              type="button"
              className="btn btn-border"
              onClick={() => setCastCharacters((prev: any) => [...prev, createCastCharacter()])}
              disabled={loading || augmentLoading}
            >
          ...
          <div style={{ border: "1px solid #d8dee8", borderRadius: 8, padding: 8, marginBottom: 8 }}>
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "0.9rem" }}>
              <input
                ...
                onChange={(e: any) => setAutoCharacterMode(e.target.checked)}
                disabled={loading}
              />
              <span>
                {t({
          ...
          <label style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
            ...
            <input
              ...
              onChange={(e: any) => toggleRandomSpeakerKey("main", e.target.checked)}
            />
            <span>{characterName?.trim() || t({ ja: "メインキャラ", en: "Main character" })}</span>
          </label>
          {castCharacters.map((cast: any) => (
            <div key={cast.key} style={{ borderTop: "1px solid #eee", paddingTop: 8, marginTop: 8 }}>
              {(() => {
                ...
                const hasCurrentInHistory = relationshipMemoOptions.some(
                  (memo: any) => memo.text === effectiveRelationship
                );
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <input
                  ...
                  onChange={(e: any) => toggleRandomSpeakerKey(cast.key, e.target.checked)}
                />
                <span>{cast.name?.trim() || t({ ja: "サブキャラ", en: "Sub character" })}</span>
              </label>
              ...
              {renderSpeechGenderButtons(
                ...
                (next: any) => updateCastCharacter(cast.key, { speech_gender: next })
              )}
              <select
                value={cast.saved_id || ""}
                onChange={(e: any) => applySavedCharacterToCast(cast.key, e.target.value)}
              ...
              <input
                value={cast.name}
                onChange={(e: any) => updateCastCharacter(cast.key, { name: e.target.value })}
                placeholder={t({ ja: "サブキャラ名", en: "Sub character name" })}
                style={{ width: "100%", marginTop: 6 }}
              />
              <textarea
                value={cast.personality}
                onChange={(e: any) => updateCastCharacter(cast.key, { personality: e.target.value })}
              ...
              <textarea
                ...
                onChange={(e: any) => updateCastCharacter(cast.key, { appearance: e.target.value })}
                rows={2}
              ...
              <div style={{ display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap", alignItems: "center" }}>
                <select
                  ...
                  onChange={(e: any) => {
                    ...
                    if (!next) return;
                    setCastRelationshipSelectMap((prev: any) => ({ ...(prev || {}), [cast.key]: next }));
                ...
                >
                  ...
                  {relationshipSelectOptions.map((memo: any) => (
                ...
                <input
                  ...
                  onChange={(e: any) => updateCastCharacter(cast.key, { relationship: e.target.value })}
                  onBlur={(e: any) => rememberRelationshipMemo(e.target.value)}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginTop: 6 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <input
                    ...
                    onChange={(e: any) => updateCastCharacter(cast.key, { fanfic_mode: e.target.checked })}
                  />
          ...
          <datalist id="relationship-memo-history">
            {relationshipMemoOptions.map((memo: any, idx: any) => (
        ...
        <label>
          ...
          <textarea
            value={appearance}
            onChange={(e: any) => setAppearance(e.target.value)}
            placeholder={t(
          ...
          <input
            type="file"
            accept="image/*"
            onChange={(e: any) => setCharacterImageFile(e.target.files?.[0] || null)}
            style={{ width: "100%", marginTop: 4 }}
          />
          <div style={{ marginTop: 6, display: "flex", justifyContent: "flex-end" }}>
            <button
          ...
          <div
            ...
            onClick={() => setSelectedMessageIndex((prev: any) => (prev === idx ? null : idx))}
          >
	            {m.mode === "do" ? (
	              <div
	                style={{
                  ...
                  {Array.isArray(m.generated_images) && m.generated_images.length > 0 && (
                    <div style={{ marginTop: m.content ? 8 : 0, display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-start" }}>
                      {Array.isArray(m.image_descriptions) && m.image_descriptions.length > 0 && (
                        ...
                        >
                          {m.image_descriptions.map((desc: any, didx: any) => (
                      ...
                      {m.generated_images.map((img: any, gidx: any) => {
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 8, padding: 10, marginBottom: 10 }}>
        ...
        <textarea
          ...
          onChange={(e: any) => setImagePromptDraft(e.target.value)}
          rows={3}
          placeholder={t({
        ...
        <textarea
          ...
          onChange={(e: any) => setImageNegativePromptDraft(e.target.value)}
          rows={2}
          placeholder={t({
      ...
      <div style={{ marginTop: -2, marginBottom: 10 }}>
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {selectedSpeakerBubbles.bubbles.map((line: any, idx: any) => {
      ...
      {lastRequest && (
        ...
        >
          ...
          <textarea
            value={resendDraft}
            onChange={(e: any) => setResendDraft(e.target.value)}
            rows={2}
            style={{ width: "100%", marginBottom: 8 }}
            disabled={loading}
          />
      ...
      <div style={{ marginTop: 8 }}>
        <label style={{ display: "flex", alignItems: "flex-start", gap: 8, fontSize: "0.92rem" }}>
          <input
            ...
            onChange={(e: any) => setAutoDialogue(e.target.checked)}
            disabled={loading}
            style={{ marginTop: 3 }}
          />
