...
type UserProfile = {
  ...
  favorite_visibility?: string | null;
...
export default function UserPage() {
  ...
  const [followListKind, setFollowListKind] = useState<FollowListKind>("");
  ...
  const [followListActionUserId, setFollowListActionUserId] = useState<number | null>(null);
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        console.error(error);
        setError(getErrorMessage(error, t({ ja: "エラーが発生しました", en: "An error occurred." })));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCreateDm = async () => {
    ...
    try {
      ...
      console.error(error);
      setDmError(getErrorMessage(error, t({ ja: "エラーが発生しました", en: "An error occurred." })));
    } finally {
      setDmLoading(false);
    }
  };
  ...
  const fetchFollowList = async (
    targetKind: Exclude<FollowListKind, "">,
    page = 0,
  ...
  const handleLoadFollowList = async (kind: FollowListKind) => {
    const targetKind: Exclude<FollowListKind, ""> = kind === "followers" ? "followers" : "following";
  ...
  const handleToggleFollowInList = async (item: FollowListItem) => {
  ...
  const renderNovelList = ({ items, emptyText, truncateDescription = true }: NovelListProps) => {
