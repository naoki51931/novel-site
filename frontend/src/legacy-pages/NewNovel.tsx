...
export default function NewNovel() {
  ...
  const handleDismissBubble = (e: MouseEvent<HTMLElement> | null, key: string) => {
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
  ...
  const handleSuggestSummaries = async () => {
    ...
    try {
      ...
      setSummarySuggestError(
        getErrorMessage(
          err,
          t({ ja: "あらすじ候補の生成に失敗しました。", en: "Failed to generate summary candidates." })
        )
      );
    } finally {
  ...
  useEffect(() => {
    try {
      ...
      if (!raw) return;
      const draft = JSON.parse(raw) as DraftNovel;
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        getErrorMessage(
          err,
          t({ ja: "小説の作成中にエラーが発生しました", en: "An error occurred while creating the novel." })
        )
      );
    } finally {
      setSaving(false);
