...
export default function NovelDetail() {
  ...
  const getNovelTags = (novelData: Novel | null, episodeList: Episode[] = []): NovelTag[] => {
  ...
  const formatEpisodeDisplayTitle = (
  ...
  const getAgeLabel = (ageLimit: string | null | undefined) => {
    if (!ageLimit) return t({ ja: "全年齢", en: "All ages" });
    return AGE_LABELS[ageLimit as keyof typeof AGE_LABELS] ?? ageLimit;
  };
  ...
  const getCreativeTypeLabel = (creativeType: string | null | undefined) => {
    if (!creativeType) return t({ ja: "オリジナル", en: "Original" });
    return CREATIVE_TYPE_LABELS[creativeType as keyof typeof CREATIVE_TYPE_LABELS] ?? creativeType;
  };
  ...
  const toggleFavorite = async () => {
    ...
    try {
      ...
      alert(
        getErrorMessage(
          e,
          t({ ja: "お気に入り操作中にエラーが発生しました", en: "An error occurred while updating favorite." })
        )
      );
    }
  };
    ...
    const fetchNovel = async () => {
      try {
        ...
        if (TRANSLATABLE_LANGS.has(lang) && data?.id) {
          ...
          if (tr) {
            data = {
              ...
              tags: Array.isArray(tr.tags) ? tr.tags.map((name: string) => ({ name })) : data.tags,
            };
          }
              ...
              data.episodes.map(async (ep: Episode) => {
                ...
                return {
                  ...
                  tags: Array.isArray(epTr.tags)
                    ? epTr.tags.map((name: string) => ({ name }))
                    : ep.tags,
                };
              })
            );
        ...
        setError(
          getErrorMessage(
            e,
            t({ ja: "小説の取得中にエラーが発生しました", en: "An error occurred while loading the novel." })
          )
        );
      } finally {
        setLoading(false);
  ...
  const handlePostComment = async () => {
    ...
    try {
      ...
      alert(
        getErrorMessage(
          e,
          t({ ja: "コメント投稿中にエラーが発生しました", en: "An error occurred while posting comment." })
        )
      );
    }
  };
  ...
  const handleToggleLike = async () => {
    ...
    try {
      ...
      alert(
        getErrorMessage(
          e,
          t({ ja: "いいね操作中にエラーが発生しました", en: "An error occurred while liking." })
        )
      );
    }
  };
  ...
  const handleDeleteNovel = async () => {
    ...
    try {
      ...
      alert(
        getErrorMessage(
          e,
          t({ ja: "削除中にエラーが発生しました", en: "An error occurred while deleting." })
        )
      );
    }
  };
    ...
    try {
      ...
      alert(
        getErrorMessage(
          e,
          t({ ja: "コメント削除中にエラーが発生しました", en: "An error occurred while deleting comment." })
        )
      );
    }
  };
  ...
  // ★ エピソード編集ボタン
  const handleEditEpisode = (episodeId: number | string) => {
	...
	{novel.created_at && (
      ...
      {tags.length > 0 && (
        <div className="tag-chip-row" style={{ marginBottom: 12 }}>
          {tags.map((tagItem: NovelTag) => (
            <TagChipLink key={tagItem.id ?? tagItem.name} name={tagItem.name} />
          ))}
        </div>
      )}
