...
type PublicChatMessage = {
  ...
  role?: string | null;
  mode?: string | null;
  is_auto_dialogue?: boolean | null;
  content?: string | null;
  is_generated_image?: boolean;
  image_message_kind?: string | null;
  generated_images?: Array<string | { url?: string | null }> | null;
  image_descriptions?: string[] | null;
};

type PublicChatCharacter = {
  id: number | string;
  name?: string | null;
  ...
  messages?: PublicChatMessage[] | null;
};

function parseAiChatImageMessageContent(content: string | null | undefined): AiChatImageMessage | null {
  ...
  try {
    ...
    const descriptions = Array.isArray(meta?.descriptions)
      ? meta.descriptions.map((v: unknown) => String(v || "").trim()).filter(Boolean)
      : [];
    return {
      kind,
      images,
...
function resolveImageUrl(rawUrl: string | null | undefined) {
...
function slugifyCharacterName(name: string | null | undefined) {
  return String(name || "")
    .normalize("NFKC")
    .trim()
    .toLowerCase()
...
export default function AiChatPublicPage() {
  ...
  const [items, setItems] = useState<PublicChatCharacter[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [selectedId, setSelectedId] = useState<number | string | null>(null);
  const [detail, setDetail] = useState<PublicChatCharacter | null>(null);
  ...
  const buildUrlSearch = (query: string | null | undefined) => {
  ...
  const search = async ({ query = q, syncUrl = true }: { query?: string; syncUrl?: boolean } = {}) => {
    ...
    try {
      ...
      setError(
        getErrorMessage(e, t({ ja: "公開チャット検索中にエラーが発生しました。", en: "Failed to search public chats." }))
      );
    } finally {
      setLoading(false);
    }
  ...
  const loadDetail = async (
    id: number | string,
    { name = "", syncUrl = true }: { name?: string; syncUrl?: boolean } = {}
  ) => {
    setSelectedId(id);
    ...
    try {
      ...
      const normalizedMessages = (Array.isArray(data?.messages) ? data.messages : []).map((msg: PublicChatMessage) => {
  ...
  const applySocialState = (id: number, patch: Partial<PublicChatCharacter>) => {
    setItems((prev) => prev.map((item: PublicChatCharacter) => (Number(item.id) === id ? { ...item, ...patch } : item)));
  ...
  const toggleFavorite = async () => {
    ...
    try {
      ...
      alert(getErrorMessage(e, t({ ja: "ブックマーク操作中にエラーが発生しました。", en: "An error occurred while bookmarking." })));
    }
  };
