...
type PretranslatedLanguage = "zh-cn" | "zh-tw" | "ko";
type SourceItem = {
  source_lang: "ja";
  text: string;
};
...
type I18nJobDetail = I18nJobSummary & {
  translated_count?: number | null;
  failed_count?: number | null;
  current_source_lang?: string | null;
  current_target_lang?: string | null;
};

function buildSourceItems() {
  const texts = new Set<string>();
  for (const lang of ["zh-cn", "zh-tw", "ko"] as const satisfies readonly PretranslatedLanguage[]) {
  ...
  return Array.from(texts).map((text): SourceItem => ({ source_lang: "ja", text }));
}
...
export default function AdminI18nJobs() {
  ...
  const [jobs, setJobs] = useState<I18nJobSummary[]>([]);
  const [activeJobId, setActiveJobId] = useState("");
  const [activeJob, setActiveJob] = useState<I18nJobDetail | null>(null);
  ...
  useEffect(() => {
    const boot = async () => {
      try {
        ...
        setError(getErrorMessage(error, t({ ja: "読み込みに失敗しました。", en: "Failed to load." })));
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    clearPoll();
    loadJobDetail(activeJobId).catch((error) => {
      setError(getErrorMessage(error, t({ ja: "ジョブ取得に失敗しました。", en: "Failed to load job." })));
    });
  }, [activeJobId]);
  ...
  const startJob = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(error, t({ ja: "開始に失敗しました。", en: "Failed to start." })));
    } finally {
      setStarting(false);
    }
  };
  ...
  const cancelJob = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(error, t({ ja: "停止に失敗しました。", en: "Failed to cancel." })));
    }
  };
  ...
  const refreshNow = async () => {
    try {
      ...
      setError(getErrorMessage(error, t({ ja: "更新に失敗しました。", en: "Failed to refresh." })));
    } finally {
      setLoading(false);
    }
  };
