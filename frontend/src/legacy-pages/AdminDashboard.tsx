...
type PayoutsData = {
  start_date?: string | null;
  days?: number | null;
  payout_minimum_yen?: number | null;
  paid_amounts?: number[] | null;
  upcoming?: PayoutItem[] | null;
  recent_paid?: PayoutItem[] | null;
};
...
const yen = (value: number | null | undefined, locale: string) =>
  new Intl.NumberFormat(locale || "ja-JP").format(value || 0);

function SparkBars({ data, width = 260, height = 46, color = "#2f6f6d" }: { data: number[]; width?: number; height?: number; color?: string }) {
...
export default function AdminDashboard() {
  ...
  const [supports, setSupports] = useState<SupportsData | null>(null);
  ...
  const [selectedProfile, setSelectedProfile] = useState<PayoutProfile | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setError(getErrorMessage(e, t({ ja: "データ取得に失敗しました", en: "Failed to load data." })));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleProfileOpen = async (authorUserId: number | string | null | undefined) => {
    try {
      ...
      setProfileError(
        getErrorMessage(e, t({ ja: "口座情報の取得に失敗しました", en: "Failed to load bank details." }))
      );
    } finally {
        ...
        <>
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <label style={{ fontSize: 13, color: "var(--muted-text)" }}>
            ...
            <div style={{ fontSize: 12, color: "var(--muted-text)", marginBottom: 12 }}>
              {supportWindow}
            </div>

            {supports?.users?.length ? (
              <div style={{ display: "grid", gap: 12 }}>
                {supports.users.map((user: SupportUser) => (
                  <div
                    key={user.user_id}
                    style={{
                      display: "grid",
            ...
            <div style={{ display: "grid", gap: 16 }}>
              <div>
                ...
                {payouts?.upcoming?.length ? (
                  <div style={{ display: "grid", gap: 6 }}>
                    {payouts.upcoming.map((item: PayoutItem) => (
                      <div
                        key={item.payout_id}
                        style={{
                          display: "grid",
                      ...
                      >
                        ...
                        <div style={{ textTransform: "uppercase", color: "var(--muted-text)" }}>
                          {item.status}
                        </div>
                      </div>
                    ))}
              ...
              <div>
                ...
                {payouts?.recent_paid?.length ? (
                  <div style={{ display: "grid", gap: 6 }}>
                    {payouts.recent_paid.map((item: PayoutItem) => (
                      <div
                        key={item.payout_id}
                        style={{
                          display: "grid",
