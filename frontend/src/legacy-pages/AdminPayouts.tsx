...
type PayoutPreviewAuthor = {
  author_user_id: number;
  username?: string | null;
  total_amount_yen?: number | null;
  support_amount_yen?: number | null;
...
type PayoutPreview = {
  ...
  authors?: PayoutPreviewAuthor[] | null;
};
...
export default function AdminPayouts() {
  ...
  const [payoutsList, setPayoutsList] = useState<PayoutListItem[]>([]);
    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        setProfileError(
          getErrorMessage(
            error,
            t({ ja: "振込先情報の取得に失敗しました", en: "Failed to load payout destination." })
          )
        );
      } finally {
  ...
  const loadPayouts = async () => {
    ...
    const items = Array.isArray(data.items) ? (data.items as PayoutListItem[]) : [];
    setPayoutsList(items);
    if (items.length && !payoutId) {
      setPayoutId(String(items[0].payout_id));
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(error, t({ ja: "精算生成に失敗しました", en: "Failed to generate payouts." })));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(error, t({ ja: "確認に失敗しました", en: "Preview failed." })));
    } finally {
      setLoading(false);
    }
  };

  const handleMark = async (action: "mark_paid" | "mark_failed") => {
    if (!payoutId) {
    ...
    try {
      ...
      setError(getErrorMessage(error, t({ ja: "更新に失敗しました", en: "Failed to update." })));
    } finally {
      setLoading(false);
    }
  };

  const yen = (value: number | string | null | undefined) =>
    new Intl.NumberFormat(lang === "en" ? "en-US" : "ja-JP").format(Number(value || 0));
            ...
            <div style={{ marginTop: 8 }}>
              {preview.authors?.length ? (
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  ...
                  <tbody>
                    {preview.authors.map((row: PayoutPreviewAuthor) => (
