...
type PendingAiPost = {
  body?: string | null;
  kind?: "new_novel" | "next_episode" | string;
  generated_title?: string | null;
  age_limit?: string | null;
  continue_novel_id?: number | string | null;
  post_episode_title?: string | null;
};

function loadPendingAiPost(): PendingAiPost | null {
  try {
...
function savePendingAiPostError(message: string) {
  try {
    ...
    try {
      ...
      if (pending.kind === "next_episode") {
        ...
        const numbers = Array.isArray(listData)
          ? listData
              .map((e: { number?: number | null }) => (typeof e?.number === "number" ? e.number : null))
  ...
  const handleVerify = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        getErrorMessage(
          err,
          t({ ja: "認証中にエラーが発生しました。", en: "An error occurred during verification." })
        )
      );
    } finally {
      setLoading(false);
  ...
  const handleOAuth = async (provider: string) => {
    ...
    try {
      ...
      setError(
        getErrorMessage(
          err,
          t({ ja: "OAuth でエラーが発生しました。", en: "An error occurred during OAuth." })
        )
      );
